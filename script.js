document.addEventListener('DOMContentLoaded', () => {
    ...
    contactForm.addEventListener('submit', async (e) => {
        ...
        let telegramMessage = `Yangi xabar!\n\n` +
                              `Ism: ${nameValue}\n` +
                              `Email: ${emailValue}\n` 
        ...
        try {
            ...
            if (response.ok) {
                alert("Muvaffaqiyat", "Xabar muvaffaqiyatli yuborildi!", true);
                contactForm.reset();
            } else {
                const errorData = await response.json();
                console.error('Telegram API Error:', errorData);
                alert("Xato", "Xabarni yuborishda xatolik yuz berdi. Iltimos, qayta urinib ko'ring.", false);
            }
        } catch (error) {
            console.error('Network Error:', error);
            alert("Tarmoq xatosi", "Tarmoq bilan bog'lanishda muammo yuz berdi. Internetingizni tekshiring.", false);
        }
    });
});